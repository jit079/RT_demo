...
# This code simulates reflectance and polarized reflectance at TOA and BOA level as a function of wavelength 400-900 nm by steps of 5 nm for a typical atmosphere and ocean containing phytoplankton and CDOM, and a typical geometry. The water model in OSOAA goes from 400 to 700 nm. Outside this range the phytoplankton absorption of forced to be zero.

# To run the code, 'bash ./run_OSOAA_example.ksh', and the simulations are saved in $OSOAA_ROOT/example_RESULTS


# OSOAA.Wa: Wavelength for radiance simulation (micron)
# ANG.Thetas: solar zenith angle in degrees
# AP.Pressure: atmospheric pressure at sea level (mbar)
# AP.HR: Height scale of the molecular profile (km)
# AER.Waref: reference wavelength (micron) for the aerosol optical thickness
# AER.AOTref: aerosol optical thickness for the reference wavelength
# AP.HA: height scale of the aerosol profile (km)
# AER.Model: type of aerosol model, 2 - Shettle &Fenn bi-modal
# AER.SF.Model: Shettle & Fenn model 1 - tropospheric
# AER.SF.RH: percentage of air relative humidity (from 0 to 99%)
# PHYTO.Chl: Chlorophyll concentration at sea surface (mg/m3)
# SED.Csed  sediment concentration (mg/l)
# PHYTO.ProfilType: 1 - homogenous profile
# YS.Abs440: absorption coefficient of yellow substance at 440 nm (m-1)
# DET.Abs440: absorption coefficient of detritus at 440 nm (m-1)
# HYD.Model: type of hydrofoil characterization (1 - from size distribution models (Mid calculations))
# PHYTO.JD.slope, PHYTO.JD.rmin, PHYTO.JD.rmax, PHYTO.JD.MRwa, PHYTO.JD.MIwa, and PHYTO.JD.rate are parameters for phytoplankton, related to HYD.Model=1
# SEA.Depth: sea depth (m)
# SEA.SurfAlb: sea surface albedo
# SEA.BotType: sea bottom type, 1 means lambertian
# SEA.BotAlb: sea bottom albedo
# OSOAA.View.Phi: relative azimuth angel (degrees) for output radiance
# OSOAA.View.Level: 1 - top of atmosphere, 3 -  over sea surface 0+
# OSOAA.View.VZA: viewing zenith angels (degrees) for which the radiance has to be given versus the depth
...
sz=30. # solar zenith angle
vz=0. # view zenith angle
ra=90. # relative azimuth

# wavelength from 400 to 900 nm, 5 nm interval
declare -a wl
...
while (( $(echo "$current <= $end" | bc -l))); do
    wl+=$(printf "%.3f " $current)
    current=$(echo "$current + $step" | bc -l)
done


tau=0.1 # AOT@550
rh=90. # relative humidity, for Shettle and Fenn model, tropospheric

chl=0.1 # chl concentration 
ys=0.03   # yellow substance and detritus absorption at 440 nm

level=(1 3) # TOA and BOA
...
# simulation will be save in ${OSOAA_ROOT}/example_RSULTS/ 
dirRESULTS=${OSOAA_ROOT}/example_RESULTS/wl${k}_level${l}  && mkdir -p ${dirRESULTS}
...
${OSOAA_ROOT}/exe/OSOAA_MAIN.exe \
	  ...
	  -AER.Waref 0.550 -AER.AOTref $tau  \
	  -AER.DirMie ${dirMIE_AER} \
	  -AER.Model 2 \
	  -AER.SF.Model 1 -AER.SF.RH $rh  \
	  -PHYTO.Chl $chl \
	  -SED.Csed  0.0 -PHYTO.ProfilType 1 \
	  -YS.Abs440 $ys  -DET.Abs440 0.00 \
	  -SEA.Depth 15.000 \
	  ...
	  -SEA.Dir ${dirSURF} -SEA.Ind 1.34  -SEA.Wind 5   \
	  -SEA.SurfAlb 0.0  -SEA.BotType 1 -SEA.BotAlb 0.30 \
 	  -OSOAA.View.Phi $ra \
	  -OSOAA.View.Level $l\
	  -OSOAA.View.VZA $vz\
          -OSOAA.ResFile.vsVZA RESLUM_vsVZA.txt \
	  -OSOAA.ResFile.vsZ RESLUM_vsZ.txt \
